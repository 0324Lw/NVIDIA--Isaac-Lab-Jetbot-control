...
python -m diff_drive_rl.tasks.task1.task1_train --num-envs $(if ($env:NUM_ENVS) { $env:NUM_ENVS } else { "512" }) --total-env-steps $(if ($env:TOTAL_ENV_STEPS) { $env:TOTAL_ENV_STEPS } else { "5000" }) --save-freq-env-steps $(if ($env:SAVE_FREQ_ENV_STEPS) { $env:SAVE_FREQ_ENV_STEPS } else { "5000" }) @args
