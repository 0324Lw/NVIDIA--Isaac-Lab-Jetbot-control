...
if (-not $CheckpointPath) {
  $CheckpointPath = Get-ChildItem -Path "logs\task1" -Recurse -Directory -Filter "final_checkpoint" -ErrorAction SilentlyContinue | Sort-Object FullName | Select-Object -Last 1 | ForEach-Object { $_.FullName }
}
if (-not $CheckpointPath) {
  Write-Error "Checkpoint not found. Set CHECKPOINT=/path/to/checkpoint"
}
python -m diff_drive_rl.tasks.task1.task1_model_test --checkpoint $CheckpointPath --visualize @args
