...
python -m diff_drive_rl.tasks.task4.task4_train --num-envs $(if ($env:NUM_ENVS) { $env:NUM_ENVS } else { "64" }) --total-agent-steps $(if ($env:TOTAL_AGENT_STEPS) { $env:TOTAL_AGENT_STEPS } else { "5000" }) --save-freq-agent-steps $(if ($env:SAVE_FREQ_AGENT_STEPS) { $env:SAVE_FREQ_AGENT_STEPS } else { "5000" }) @args
