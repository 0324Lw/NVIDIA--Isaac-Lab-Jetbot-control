...
class DiffDriveTask1SkrlWrapper(gym.Env):
    ...
    The underlying environment already outputs the 3-frame stacked 36-D
    observation. For skrl asymmetric-compatible format, policy and critic both
    receive the same 36-D tensor in Task1.
    """
...
class DiffDriveCritic(DeterministicMixin, Model):
    def __init__(self, observation_space, state_space, action_space, device):
        ...
        self.net = nn.Sequential(
            nn.Linear(state_space.shape[0], 256),
            nn.ELU(),
            nn.Linear(256, 128),
            nn.ELU(),
            nn.Linear(128, 64),
...
def main() -> None:
    ...
    env_cfg.num_waypoints = int(args_cli.num_waypoints)
    ...
    print("👉 Actor/Critic 输入：3 帧堆叠观测，36 维")
    print("👉 动作：2 维左右轮速度控制")
    print("👉 日志重点：Progress / Goal_Aligned_Speed / Waypoint_Index / Finish_Rate / Timeout_Rate / Stuck_Ratio\n")
